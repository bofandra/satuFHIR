...
app.post('/api/rme/:kind',zValidator('param',z.object({kind:z.enum(['chl','shl'])})),zValidator('json',rmeSchema),async c=>{const{kind}=c.req.valid('param');try{return c.json(await rmeLink(c,kind,c.req.valid('json')))}catch(e:any){return c.json({message:e.message},400)}})
